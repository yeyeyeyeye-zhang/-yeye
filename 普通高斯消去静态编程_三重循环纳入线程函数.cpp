...
//定义信号量
sem_t sem_leader;
sem_t sem_Division[NUM_THREADS-1];//每个线程有自己专属的信号量
...
void* threadFunc(void* param)
{
    ...
    for(int k=0;k<n;++k)
    {
        ...
        else{sem_wait(&sem_Division[t_id-1]);}//阻塞，等待主线程完成除法操作
        if(t_id==0)
        {
            for(int i=0;i<NUM_THREADS-1;++i)
            {
                sem_post(&sem_Division[i]);//主线程完成除法操作后唤醒其他线程
            }
        }
        ...
        {
            sem_post(&sem_leader);//通知leader已完成消去任务
...
int main()
{
	...
	while(allDuration<0.5)
	{
        ...
        //消去过程
        //初始化信号量
        sem_init(&sem_leader,0,0);//进程内线程共享，初始值为0
        for(int i=0;i<NUM_THREADS-1;++i)
        {
