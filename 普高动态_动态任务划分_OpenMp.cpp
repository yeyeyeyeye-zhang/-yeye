...
int main() {
    ...
    while (allDuration < 10) {
        ...
        //     for (int i_block = (k + 1) / 4; i_block < n / 4; i_block++) {
        //         for (int j_block = (k + 1) / 4; j_block < n / 4; j_block++) {
        //             for (int i = i_block * 4; i < (i_block + 1) * 4 && i < n; i++) {
        //                 __m128d A_ik, A_kj, product, result;
        //                 A_ik = _mm_load1_pd(&A[i][k]);
        //                 b[i] -= A[i][k] * b[k];
        //                 for (int j = j_block * 4; j < (j_block + 1) * 4 && j + 1 < n; j += 2) {
        //                     A_kj = _mm_loadu_pd(&A[k][j]);
        //                     product = _mm_mul_pd(A_ik, A_kj);
        //                     result = _mm_loadu_pd(&A[i][j]);
        //                     result = _mm_sub_pd(result, product);
